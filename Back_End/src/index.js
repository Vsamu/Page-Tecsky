...
app.post('/addcar', async function (req, res) {
    const datos = req.body;
    if (datos.idCar.length < 6) {
        return res.send({
            data: 'Debe ser mayor o igual a 6 caracteres'
        });
    }
...
app.post('/signup', async (req, res) => {
    const { firstName, lastName, email, password, idCar, cedula, ciudad, marca, cilindraje } = req.body;
    const autos = JSON.stringify({ idCar, marca, cilindraje });
    ...
    con.query("SELECT * FROM users WHERE `email` = ?", [email], function (err, results) {
        if (err) {
            ...
            if (results[0] == null) {
                console.log("es nulo")
                var con1 = dbConnections();
                var sql = "INSERT INTO `users` (`ID`, `firstName`, `lastName`, `email`, `password`, `idCar`, `cedula`, `ciudad`, `autos`) VALUES ?;";
                values = [['NULL', firstName, lastName, email, passwordE, idCar, cedula, ciudad, autos]];
    ...
    con.query('SELECT * FROM users where email = ? order by ID desc limit 1', [datos.email], function (err, resultss) {
        if (resultss[0] !== undefined) {
            ...
            for (var i = 0; i < rr.length; i++) {
                console.log(rr[i], "==", datos.idCarRem)
                if (rr[i] == datos.idCarRem) {
                    si = true;
                }
            }
            console.log("resultss", si);
            if (!si) {
                return res.send({
                    data: 'Auto no existe'
                });
